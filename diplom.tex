\documentclass[a4paper, 12pt, DIV=calc, version=first, pdftex, headsepline, footsepline, bibtotocnumbered, liststotocnumbered]{scrreprt}
...
Informationsvisualisierung bezeichnet werden. In der Fassung von \citep{BenFry} befindet sich der Aufgabenbereich
der Software in den Stufen \textit{present} und \textit{refine}. Die Software erlaubt dem Nutzer beide Stufen
bearbeiten zu können. In Abbildung \ref{fig:benfryprocess_marked} ist dieser weitere iterative Schritt
abgebildet. Die hohe Anzahl der Beobachtungspunkte eines Datensatzes macht eine vom Nutzer manuell durchgeführte Transkription
...
Daten und grafischen Darstellungsformen. Dazu kommen neue hybride Formen durch Mashups\footnote{"`Mashup (von
...
Die Software muss möglichst universell sein um flexibel viele unterschiedliche Anwendungsfälle
...
In der Prozesskette wird der generische Arbeitsablauf eines Benutzers
der Software beschrieben. Dieser ist unabhängig von den zu bearbeitenden Daten
...
in das Programm zu laden. Unterschiedliche Nodes machen es möglich Datenquellen auf
dem Computer oder im Netzwerk einlesen zu können. Dabei werden etablierte
Datenformate wie XML, CSV, JSON und netCDF unterstützt. Wenn der Benutzer eine 
...
elf Spalten und der Node würde elf Output-Pins besitzen. Diese können dann unterschiedlich
...
Der Benutzer hat nun die Möglichkeit weitere Nodes anzulegen, die die visuellen
Variablen darstellen. Diese können unabhängig voneinander erzeugt und mit einem datengebenden
Node verbunden werden. Der Benutzer erhält die Freiheit zwischen datengebenden Nodes und
...
Node erzeugen, der Rechtecke erzeugen kann. Dieser Node hat Input-Pins um die Position
...
und erzeugt damit eine erste grafische Darstellung der Daten.
...
erzeugen, die eine Art Vorschau ermöglichen. Ein Render-Node erzeugt ein weiteres
Ausgabefenster und zeigt die Ausgabe, die von den vorhandenen Nodes erzeugt wird.
...
Variablen-Node dazu genutzt werden um den Radius des Kreises anpassen zu können.
...
sind Vektorgrafiken oder interaktive HTML Seiten.
...
\hline
Add & Addiert zwei Spreads miteinander und gibt eine Summe dieser zurück.\\
\hline
Substract & Zieht vom Spread der am ersten Input-Pin anliegt, den Spread vom zweiten Input-Pin ab und gibt die Differenz aus.\\
\hline
Multiply & Multipliziert zwei Spreads und gibt das Produkt aus.\\
\hline
Divide & Teilt den Spread am ersten Input-Pin durch den Spread der am zweiten Input-Pin anliegt und gibt das Ergebnis aus.\\
\hline
...
\hline
RGBColor & Erzeugt aus drei Input-Pins (Rot, Grün, Blau) einen Spread mit Farbwerten. Dieser kann von anderen Nodes benutzt werden,
um ihre Ausgabe einzufärben.\\
\hline
Line & Dieser Node zeichnet eine Linie. Die Linie ist bestimmt durch ihre Anfangskoordinaten und ihre Endkoordinaten und kann eine Farbe annehmen.\\
\hline
Circle & Dieser Node zeichnet einen Kreis. Der Kreis ist bestimmt durch Koordinaten, Radius und Farbe.\\
\hline
Render & Diese Node erzeugt ein weiteres Fenster, in dem der Benutzer die aktuelle Ausgabe überprüfen kann.
Wenn er mit der aktuellen Ausgabe zufrieden ist, kann er durch Tastendruck den Render-Node veranlassen ein
hochauflösendes Bild zu exportieren.\\
\hline
\end{tabular}
\caption{visuelle Variablen und ihre Umsetzung in Datasynth}
\label{tab:nodes}
\end{table}
In Tabelle \ref{tab:nodes} werden einzelne Nodes exemplarisch aufgeführt und ihre Funktion beschrieben.
...
In Abschnitt \ref{sec:ImplementierteNodes} wurden alle Nodes aufgelistet,
die für den Prototyp entstanden sind. Mit diesen können erste komplexe Visualisierungen\footnote{Vgl. Abschnitt
...
und die Funktion der Rotation wurde nicht umgesetzt. Die fehlende Funktionalität kann aber
...
Divide & Teilt den Spread am ersten Input-Pin durch den Spread der am zweiten Input-Pin anliegt und gibt das Ergebnis aus.\\
\hline
Modulo & Berechnet den Modulo zweier Spreads.\\
\hline
...
um ihre Ausgabe einzufärben.\\
\hline
Text & Gibt dem Nutzer die Möglichkeit Text einzugeben. Dieser Text wird über den Ouput-Pin des Node ausgegeben.\\
\hline
