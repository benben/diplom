...
\begin{document}
\title{Non-Standard-Informationsvisualisierer mit openFrameworks}
...
http://germany.emc.com/about/news/press/2011/20110628-01.htm
http://germany.emc.com/collateral/analyst-reports/idc-extracting-value-from-chaos-ar.pdf
noch einbauen

Die Menge an digital gespeicherten Daten nimmt stetig zu. In nahezu allen Bereichen unseren
...
Internet oder in der Bibliothelk können mit Visualisierung unterstüzt werden und müssen somit auch
für ungeübte Anwender verständlich und benutzbar sein. Dabei sollte die Visualisierung ansprechend
sein und damit eine angenehme und ästhtisch ansprechende Benutzung gewährlsieten zu können aber
gleichzeitig auch intuitiv sein, um ohne Vorkenntnisse nutzen aus dieser Form der
Mensch-Maschine-Interaktion ziehen zu können. So kann die tägliche Arbeit unterstützt werden, damit
es Gelegenheitsanwendern leicht gemacht wird, Informationen und Wissen noch besser mit anderen
teilen zu können. Somit liegt es an den Tools, die den menschen unterstützen und die möglichkeit
geben, diese daten für ihre jeweiligen zwecke nutzen zu können. dabei gibt es eine vielzahl von
unterschiedlichen problemen und fragestellungen die gelöst werden müssen.
...
immer komplexere Datenstrukturen nicht mehr mit klassischen Softwarelösungen wie Tabellenkalkulation
einfach dargstellt werden. Man braucht einen neuen tiefergehenden Ansatz und die dazugehörige
software um das Wesen der Daten wieder begreifbar zu machen.
Es entstehen Softwarelösungen die auf Teilbereiche wie beispielsweise der Medizin und Biotechnologie
oder der GIS spezialisiert sind oder es entwickeln sich vom Benutzer selbsterstellte Workflows um
Visualisierung umzusetzen. Diese Anwendungen sind nur von Wissenschaftlern oder spezialisierten
Experten verwendbar und erfordern ein hohes Maß an Vorwissen und Zeit. Dabei wird auf Tools
zurückgegriffen die aus verwandten Bereichen, wie beisüpielsweise der Grafiksoftware stammen.
Dadurcherhöht sich der Arbeitsaufwand und die Qualität leidet. Bis heute existiert keine
...
\chapter{Grundlagen}
In diesem Teil der Arbeit wird auf die grundlegenden Begriffe eingegangen die einen Zugang zum Arbeitsfeld der Visualisierung voraussetzen.
\section{Begriffsdefinition Daten und Information}
Daten werden im allgemeinen Bereich der Informatik als "zum Zweck der Verarbeitung zusammengefasste Zeichen, die aufgrund bekannter oder unterstellter Abmachungen Informationen (d.h. Angaben über Sachverhalte und Vorgänge) darstellen"[http://wirtschaftslexikon.gabler.de/Archiv/54483/daten-v5.html], beschrieben. Damit ähnelt sich die Definition von Gabler mit der Definition des ISO 2382 Standards. Daten sind also die Träger der Information. So ist der Anwender nicht direkt an den Daten als solceh interessiert sondern an den Informationen, die er mit verschiedenen Techniken aus diesen Daten gewinnen kann. Die Visualisierung ist eine dieser Techniken und ist dadurch auf eine Klassifikation von Daten angewiesen (siehe folgene Kapitel).
...
Die Begriffe Daten und Information werden nicht immer eindeutig verwendet und eine genaue beschreibung dieser steht noch aus (informationstheorie). In dieser Arbeit wird von Daten im allgemeinen und von Information als eigentlicher Ausdruck der Daten gesprochen. Deshlab auch Infovis anstatt Datavis
Somit adressiert die Informationsvisualisierung einen Spezialfall der Datenvisualisierung, der alle Anforderungen an Datenvisualisierung enthält und mit zusätztlichen Anforderungen erweitert.

In dieser Arbeit wird Visualisierung synonym mit der Informationsvisualisierung verwendet um 

Der Begriff der Daten steht in dieser Arbeit synonym zur Information, die diese Daten enthält und dient zur Klassifikation der zugrundeliegenden Daten.
...
\chapter{Forschungsfeld Informationsvisualisierung}
