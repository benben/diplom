...
Staaten wie Großbritannien verfolgen ein ganzheitliches Konzept des e-Government. Um diesen
Entwicklungen Rechnung zu tragen, ist es notwendig neue Strategien zu entwickeln, die diese Daten
für die jeweiligen Zielgruppen sinnvoll extrahieren, aufzuarbeiten und darzustellen. Aus diesem
Grund haben sich in den letzen 50 Jahren verschiedene Forschungsfelder, wie Statistik, Data Mining,
Computergrafik und Informatik entwickelt. Ein Forschungsgebiet verbindet all diese Felder um vor
allem den aktuellen Entwicklungen unserer digitalen Welt Rechnung zu tragen: die Informationsvisualisierung.
...
Rückschlüße auf den eigentlichen Forschungsgegenstand ziehen zu können. Ursprünglich aus dem rein
wissenschaftlichen Kontekt kommend, zeichnet sich eine Entwicklung hin zu einem populäreren Arbeitsfeld ab
...
werden, addressieren diese nicht mehr nur Experten und Wissenschaftler. Auch tägliche Anwendungen
im Bereich der soziale Netzwerke, beim Einkauf Online oder im Supermarkt, bei die Suche nach Informationen
im Internet oder in der Bibliothelk können mit der Informationsvisualisierung unterstüzt
werden und werden damit auch für ungeübte Anwender verständlich und benutzbar. Dabei sollte
die Informationsvisualisierung ansprechend sein um eine angenehme und ästhetisch ansprechende Benutzung
...
die den Menschen unterstützen und ihm die Möglichkeit geben, diese Daten für ihre Problemlösung nutzen zu können.
...
mathematische, informationstechnische und gestalterische Anforderungen bedacht und in Einklang gebracht
werden müssen. Da die Entwicklung unserer Informationsgesellschaft viel schneller voranschreitet als
die Entwicklung neuer Werkzeuge, müssen neue Wege gefunden werden, um das Verständnis des eigentlichen
Rezipientenweiterhin gewährlsiten zu können. Immer komplexere Datenstrukturen können nicht mehr mit
aktuell vorhandenen klassischen Softwarelösungen wie beispielsweise Tabellenkalkulationssoftware
einfach verarbeitet werden. Stattdessen wird ein neuer grundlegender Ansatz um den eigentlichen
Inhalt der Daten, die Information, wieder begreifbar zu machen.
Es entstehen Softwarelösungen die auf Teilbereiche der Informationsvisualisierung wie beispielsweise der Medizin,
Biotechnologie oder der Geoinformationssysteme spezialisiert sind. Diese Anwendungen sind aber nur von
Wissenschaftlern oder spezialisierten Experten verwendbar und erfordern ein hohes Maß an Vorwissen
und Einarbeitungszeit. Anwender aus Bereichen der Journalistik und Kunst entwickeln eigene Workflows
um eine Informationsvisualisierung umzusetzen.
Dabei wird auf Tools zurückgegriffen die aus verwandten Bereichen stammen, wichtige Aspekte der
Informationsvisualisierung aber vernachlässigen.
Dadurch erhöht sich der Arbeitsaufwand und die Qualität sinkt. Bis heute existiert keine
Softwarelösung die eine universelle Möglichkeit bietet alle Formen und Anwendungsbereiche der
Informationsvisualisierung bearbeiten zu können um eine jeweils für die Anwendung und den Zielgruppe
adäquates Ergebnis zu liefern.
In dieser Arbeit soll ein Lösungsversuch untersucht werden, der auf einen grundlegenden und
abstrakteren Ansatz der Informationsvisualisierung abzielt, als es bisherige Softwarelösungen getan haben.
Ergänzend dazu wird ein Prototyp, der die Idee beispielhaft implementiert.
Diese Software kann mithilfe von visueller Programmierung\footnote{Vgl. Abschnitt \ref{sec:visPro}}
bedient werden, welches ein breites Spektrum an Nutzergruppen zulässt. 

\section{Aufbau der Arbeit}
Um einen Lösungansatz für das Problem der einfachen aber universellen Informationsvisualisierung
zu finden, werden in dieser Arbeit Schritte durchgeführt, die die Implementation eines
Prototypen zum Ziel haben.

In Kapitel \ref{cha:Grundlagen} im Abschnitt \ref{sec:DatenInfo} werden die Begriffe Daten und
Informationen definiert und im Kontext der Informationsvisualisierung betrachtet. Dabei wird anhand
des aktuellen Forschungsstandes gezeigt, dass diese zwei Begriffe nicht immer klar voneinander abgrenzbar
sind und es in Bezug auf das jeweilige Arbeitsfeld unterschiedliche Interpretationen möglich sind.
...
Abschnitt \ref{sec:graphischeSemiologie} beschreibt ein System zur Klassifikation von grafischen Variablen,
die grundlegenden Elemente einer graphischen Darstellung.

Nachdem die Ausgangsbedingungen definiert sind wird im Kapitel \ref{cha:Informationsvisualisierung} die
Informationsvisualisierung beschrieben. Abschnitt \ref{sec:Definition} beschreibt den Begriff
der Informationsvisualisierung und nennt die Unterschiede und Besonderheiten im Vergleich zur technisch-wissenschaftlichen
Datenvisualisierung. Danach folgt im Abschnitt \ref{sec:Arbeitsfelder} eine Beschreibung der verwandten
Arbeitsfeldern, die mit der Informationsvisualisierung arbeiten oder Teilbereiche der Informationsvisualisierung
sind. Welche Ziele eine Informationsvisualisierung hat und wie man diese klassifizieren und berwerten kann wird
im Abschnitt \ref{sec:Ziele} behandelt. Im folgenden Abschnitt \ref{sec:Pipeline} wird der eigentliche Arbeitsablauf
einer Informationsvisualisierung anhand verschiedener Arbeitsschritte beschrieben um im nachfolgenden Abschnitt
\ref{sec:Darstellung} die graphische Darstellung und deren Formen als Ergebnis dessen zu zeigen.

Das Kapitel \ref{cha:Software} beschreibt nun die Konzeption einer Informationsvisualisierungssoftware.
Dabei wird im Abschnitt \ref{sec:Grundprinzip} nocheinmal das Grundprinzip der Informationsvisualisierung
...
Im Kapitel \ref{cha:Umsetzung} wird die Implementation des Prototyps vorgenommen. Dabei wird in Abschnitt
...
\label{sec:DatenInfo}
Daten werden in \cite{Gabler} als
\begin{quote}
...
\end{quote}
beschrieben.
Betrachtung dieser Arbeit sollen vor allem digital gespeicherten Daten sein.
...
Daten können dabei aus der realen Welt stammen, erhoben durch
die Messung einer physikalischen Größe oder durch Speicherung digital erzeugter Daten.
...
in verschiedenen Formen ausgelegt. Im philosophischen Bereich der Erkenntnistheorie wird versucht
einen allgemeingültigen Informationsbegriff zu definieren. Trotzdessen kann noch nicht von einer
...
In Bezug auf grapische Darstellungen wird in \citep{Bertin} die Information als der transkribierbare
...
Aufgabe, diese Information für den Menschen zu aufzudecken und in verständlicher Form aufzubereiten.
Es wird eine Transformation der Information aus den reinen Daten, die zwar diese Informationen enthalten,
aber für den menschlichen Betrachter nicht erfassbar sind, in ein leicht verständliches und intuitives
...
\cite{Schumann} beschreibt ein Modell der Eigenschaften von Daten. Jeder Datensatz\footnote{Menge von Daten}
...
mehr als drei Dimensionen besitzen. Alle Dimensionen die den Beobachtunsgraum aufspannen,
können als unabhängige Variablen angesehen werden. Als Beispiel kann die Messung in einem See oder
der Betrachtung einer bestimmten Bevölkerungsgruppe genannt werden.
...
Als Beispiel kann das eine Person einer Bevölkerungsgruppe sein.
\item[Merkmal]
...
Gemessene Vitalwerte einer Person können ein Beispiel für Merkmale sein.
\item[Ausprägung]
Die Ausprägung beschreibt den Wertebereich und damit alle Werte, die ein Merkmal annehmen kann.
Dabei sind die Datentypen Skalar, Vektor und Tensor möglich\footnote{Erklärung Skalar, Vektor, Tensor}.
Als Beispiel kann hier die Körpertemperaturwerte von einer Person sein, die einen bestimmen Wertebereich
haben.
\end{description}
Diese Eigenschaften können als Metadaten bezeichnet werden und beschreiben die Daten.
...
Anordnung und Wirkunsgkreis der Beobachtungspunkte sind beliebig aber nicht vernachlässigbar.
...
Außerdem kommt dem Geltungsbereich eine besonder Bedeutung zu, da dieser hier auf garkeinen Fall
vernachlässigt werden darf, um Fehlinterpretationen zu vermeiden.
...
bilden eine Sonderform der Daten, da weder ein Raum noch ein Zeitbezug möglich oder sinnvoll ist. Oft dienen
...
Projekte."'\citep{Gitta} }. Dies ist ein Teilgebiet der Geographie, das erste Anwedungsgebiet der
Informationsvisualisierung.
Diese Theorie definiert ein "`Grundalphabet"' an graphischen Elementen, wobei zur Informationsübertragung
...
graphische Darstellung, eindeutig beschreibt.
...
die bestimmte Zeichen in der graphischen Darstellung haben. Erst dann kann über die Verbindung der Zeichen untereinander dikstutiert
...
transkribiert werden. Dieses System umfasst folgende acht Zeichen, die zur Kodierung von Daten in eine graphische Darstellung benutzt
...
\end{itemize}
Zusätzliche Regeln sind stark abhängig von der Anzahl
...
die graphische Darstellung, als Ziel der Informationsvisualisierung, zu konstruieren.\footnote{Eine Definition
...
Fragen zu finden, die durch ein größeres Spektrum an unterschiedlichen abstrakten Daten entstehen.
\citep[S.\,434]{Preim} definiert Informationsvisualisierung folgendermaßen:
\begin{quote}
...
sozialen Netzwerken und Fahrplänen verwendet.
...
zu physikalischen Daten kein "mentales Bild"\footnote{Mentales Bild ist ein ...} existiert.
Es besteht keine Möglichkeit um durch räumliche Metaphern einen Ortsbezug herzustellen.
...
um graphische Konzepte zu realisieren, da die Informationen primär visuell übertragen werden.
...
wird mithilfe der Informationsvisualisierung eine Hypthese entwickelt. Dazu muss eine geeignete weit gefasste und nicht
...
Um eine adäqute Informationsvisualisierung klassifizieren zu können, nennen \citep{Schumann} drei Eigenschaften.
...
Prägnanz wird dabei wie folgt definiert:
\begin{quote}
...
der eine Erweiterung der Informationsvisualisierung darstellt. Abbildung \ref{fig:benfryprocess} zeigt sind
...
ausschließlich vom Begriff der "`graphischen Darstellung"' und grenzt diesen von
...
Anforderungen beachtet werden, um die effizienteste Technik der graphischen Darstellung zu
...
Visualisierungspipeline bezeichnet wird. Bevor es zu diesem Schritt kommen kann, ist es aber umso komplexer
...
Die Software muss dazu möglichst generisch sein um flexibel viele unterschiedliche Anwendungsfälle
...
graphischen Darstellung abbilden. Somit können trotz dieses Low-Level-Ansatzes große Mengen an Datenpunkten
...
des Visualisierungsprozesses konzentriert werden, um den grundlegenden Ansatz zu demonstrieren zu können. Dabei werden
...
Der Prozess des Mappings kann stark beeinflusst, woraus ein iterativer Arbeitsprozess entsteht.
...
oder welche zusätzlichen Parameter einzelne Verbindungen noch brauchen um eine sinnvolle graphische 
...
Zur Speicherung der graphischen Darstellungen einer Informationsvisualisierung eignen sich bekannte
...
allgemeine Anforderungen an Bentuzerführung und Performance gestellt.
\begin{description}
